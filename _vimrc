...
function! s:OutlineHelper(helper, ...) abort
  ...
  function! l:dict_func['SearchLine']() abort
    ...
    " <c-cr>
    if l:GET_INPUT
      unsilent const l:JUMP_TO_WINDOW = str2nr(input('Search in window? '))
    ...
    const l:SEARCH_TEXT = <sid>EscapeString(
        \ substitute(l:CURRENT_LINE, '\v^\s*(\S.{-})\s*$', '\1', ''),
        \ 0)
...
function! s:SetBufferKeyMap(file_type) abort
  ...
  function! l:dict_func['bufl']() abort
    ...
    nnoremap <buffer> <silent> o :call <sid>BufferListHelper('OpenByPrompt')<cr>
...
function! s:QuickSearch(add_to_register) abort
  ...
  function! l:dict_func['_QuickSubstitute'](input) abort
    ...
    const l:COMMAND = '%s/' .. l:NEW_PATTERN .. '/' .. l:NEW_STRING .. '/gce'
...
function! s:InsertTimeStamp(show_mode = 0, always_insert = 0,
  ...
  call <sid>SaveRestoreView(0)
  if a:show_mode ==# 0
    const l:TIME = l:dict_func['_InsertTimeStamp'](a:always_insert, a:keyword)
...
function! s:LoadAbbreviationDict(file_type) abort
  ...
  for l:i in keys(l:ABBREVIATION)
    execute 'iabbrev <silent> <buffer> ' .. l:i .. ' ' .. l:ABBREVIATION[l:i]
  endfor
endfunction
...
function! s:FormatText(line_space = 2) abort
  ...
  function! l:dict_func['_DeleteBlankLine'](line_space = 0) abort
    " If the placeholder is a non-blank string, use these search patterns. You
    " may also need to check if the placeholder already exists in a file.
    ...
    " Comment this line if the placeholder is a non-blank string and you want to
    " keep existing trailing spaces as is.
    ...
    " 2. For every text line, mark itself and following `a:line_space` lines.
    execute 1
...
function! s:LocalizationHelper(helper, ...) abort
  ...
  function! l:dict_func['CopyGlossary']() abort
    ...
    if l:MAP_MODE ==# 0
      ...
      let s:LocalizationHelper_source = substitute(l:CURRENT_LINE, l:PATTERN,
          \ '\1', '')
      let s:LocalizationHelper_target = substitute(l:CURRENT_LINE, l:PATTERN,
          \ '\2', '')
    elseif l:MAP_MODE ==# 1
      const l:EOL = "\n"
      unsilent const l:INPUT = input(
...
function! s:JumpToWindowByName(buffer_name, is_equal = 0, show_mode = 0) abort
  ...
  if a:show_mode ==# 0
    ...
    return l:dict_func['_OpenBuffer'](a:buffer_name, a:is_equal, a:show_mode)
  endif
endfunction
